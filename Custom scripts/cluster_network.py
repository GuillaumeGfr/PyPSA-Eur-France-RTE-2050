...
if __name__ == "__main__":
    ...
    #Edit for RTE 2050: Adding interconnections
    import geopandas as gpd

    nuts1 = gpd.read_file(snakemake.input.nuts1_regions)
    if "CNTR_CODE" in nuts1.columns:
        nuts1 = nuts1[nuts1["CNTR_CODE"] == "FR"]

    gdf_buses = gpd.GeoDataFrame(
        nc.buses.copy(),
        geometry=gpd.points_from_xy(nc.buses["x"], nc.buses["y"]),
        crs="EPSG:4326"
    )

    gdf_buses = gpd.sjoin(gdf_buses, nuts1[["geometry", "NUTS_ID"]], how="left", predicate="intersects")
    nc.buses["nuts1"] = gdf_buses["NUTS_ID"]


    new_nodes = {
    "DE": (9.546893, 49.299286, "DE"),
    "IT": (8.839935, 45.114930, "IT"),
    "ES": (-1.797666, 40.772670, "ES"),
    "BE": (5.828408, 51.755580, "BE"),
    "GB": (-1.059400, 51.412301, "GB"),
    "CH": (7.081395, 46.595944, "CH"),
    "IE": (-9.518025, 51.844988, "IE"),    
    }
    
    nc.buses["exclude_from_renewables"] = False

    for bus, (x, y, cntr) in new_nodes.items():
        nc.add("Bus",
            bus,
            x=x,
            y=y,
        ...
        nc.buses.loc[bus, "exclude_from_renewables"] = True
    ...
    FR_BUS = nc.buses.loc[nc.buses.country == "FR"].index[0]


#create one link per country

    for c, base_cap in power_per_link.items():
        if c == FR_BUS:
            continue                      # skip France itself

        cap = base_cap * link_counts[c]   # MW
        name = f"{c}-{FR_BUS}_link"

        if name in nc.links.index:
        ...
        nc.add(
            ...
            p_min_pu=-1.0,
            efficiency=1.0,
            carrier="cross-border"
        )

    #Country level import-export time-series

    exchange_ts = pd.read_csv(
        snakemake.input.exchange,
        index_col=0,
        parse_dates=True
    )

    #Update timezone info
    exchange_ts.index = exchange_ts.index.tz_localize(None)
    nc.set_snapshots(exchange_ts.index)

    for country in exchange_ts.columns:

        #Timestamps are from the perspective of the other countries: when the value is positive, they are importing.
        series     = exchange_ts[country]
        load_series = series.clip(lower=0.0)  
        gen_series  = (-series).clip(lower=0.0)

        #Generator capacity
        p_nom_export = float(gen_series.max())

        #Export generator
        nc.add(
            "Generator",
            name=f"{country}_export",
            bus=country,
            p_nom=p_nom_export,
            p_set=gen_series,
            carrier="imports",
        ...
        nc.add(
            "Load",
            name=f"{country}_import",
            bus=country,
            p_set=load_series
        )

    # any existing “exclude_from_renewables” column can be cast to int if needed
    if "exclude_from_renewables" in nc.buses.columns:
        nc.buses["exclude_from_renewables"] = nc.buses["exclude_from_renewables"].astype(int)
