...
def attach_load(
    ...
    #Edit for RTE 2050: Alignment to snapshots
    load.index = pd.to_datetime(load.index)
    #Drop timezones
    if isinstance(load.index, pd.DatetimeIndex) and load.index.tz is not None:
        load.index = load.index.tz_convert("UTC").tz_localize(None)
    #Reindex on exactly n snapshots
    load = load.reindex(index=n.snapshots).fillna(0)
...
def attach_wind_and_solar(
    ...
    for car in carriers:
        ...
        fr_buses = set(n.buses.index[n.buses.country == "FR"])

        with xr.open_dataset(profile_filenames["profile_" + car]) as ds:
            ds = ds.sel(bus=list(fr_buses.intersection(ds.indexes["bus"])))
            ...
            #Edit for RTE 2050: exclude foreign buses from renewables allocation
            if "exclude_from_renewables" in n.buses.columns:
                excl = set(n.buses.index[n.buses["exclude_from_renewables"].fillna(False).astype(bool)])
                p_max_pu = p_max_pu.loc[:, [b for b in p_max_pu.columns if b.split()[0] not in excl]]
                bus_bins = [bb for bb in bus_bins if bb.split()[0] not in excl]
                buses = [b for b in buses if b not in excl]
...
def attach_hydro(
    ...
    **params :
    ...
    is_fr = ppl.bus.isin(fr_buses)
    ror   = ppl.query('carrier == "ror"  and bus in @fr_buses')
    phs   = ppl.query('carrier == "PHS"  and bus in @fr_buses')
    hydro = ppl.query('carrier == "hydro" and bus in @fr_buses')
...
#Edit for RTE 2050: Add custom renewables from RTE
def assign_custom_renewables_from_csv(n, run_name, rte_regions_path):
    
    """
    EDIT for RTE 2050
    Assign renewable installed capacities to generators by region and technology,
    based on a scenario CSV and p_nom_max-based distribution.

    Parameters
    ----------
    n : pypsa.Network
    filepath : str
        Path to CSV file with columns: scenario, region, carrier, capacity_MW
    run_name : str
        Scenario name to filter

    Returns
    -------
    None – modifies n.generators in place
    """
    import pandas as pd
    
    fr_buses = set(n.buses.index[n.buses.country == "FR"])
    df = pd.read_csv(rte_regions_path)
    df = df[df["scenario"] == run_name]
    ...
    for _, row in df.iterrows():
        ...
        if carrier == "offwind":
            techs = ["offwind-ac", "offwind-dc", "offwind-float"]
        else:
            techs = [carrier]

        gen_i = n.generators.query("carrier in @techs").index
        gen_i = gen_i[n.generators.loc[gen_i, "bus"].isin(fr_buses)]
        mask = (n.generators.loc[gen_i, "bus"]
                .map(n.buses["nuts1"])
                .isin(regions))
        gen_i = gen_i[mask]

        if gen_i.empty or cap <= 0.0:
            continue

        weights = n.generators.p_nom_max[gen_i]
        weights /= weights.sum()

        n.generators.loc[gen_i, "p_nom"] = cap * weights
        n.generators.loc[gen_i, "p_nom_min"] = n.generators.loc[gen_i, "p_nom"]
        n.generators.loc[gen_i, "p_nom_extendable"] = False
...
#Edit for RTE 2050: Assign minimum flexibility capacity to each bus
def allocate_with_minimum(weights, total_capacity, min_threshold):
    """
    Distributes total_capacity across buses in 'weights', ensuring
    each gets at least min_threshold if included.
    Returns a dict: {bus_name: allocated_capacity}
    """
    eligible = weights[weights > 0].sort_values(ascending=False)

    # Step 1: allocate minimum to each
    allocations = {}
    total_needed = min_threshold * len(eligible)

    # Step 2: if total_needed > total_capacity, reduce recipients
    while total_needed > total_capacity and len(eligible) > 0:
        eligible = eligible[:-1]  # drop lowest-weight bus
        total_needed = min_threshold * len(eligible)

    if len(eligible) == 0:
        return {}  # can't assign to any bus

    # Step 3: assign min_threshold to each selected
    for bus in eligible.index:
        allocations[bus] = min_threshold

    # Step 4: redistribute remainder by weight
    remainder = total_capacity - min_threshold * len(eligible)
    if remainder > 0:
        w_normalized = eligible / eligible.sum()
        for bus in eligible.index:
            allocations[bus] += remainder * w_normalized[bus]
...
def add_flexibility(n, flexibility_csv, scenario_name):
    ...
    df = pd.read_csv(flexibility_csv)
    df = df[df["scenario"] == lookup_scenario]
    if df.empty:
        raise ValueError(f"No flexibility rows for '{lookup_scenario}'")

    
    load_buses = [b for b in n.loads.bus.unique() if b in n.buses.index]
    fr_load_buses = n.buses.loc[load_buses]
    fr_load_buses = fr_load_buses[fr_load_buses.country == "FR"].index
    load_profile = n.loads_t.p_set[fr_load_buses].mean(axis=0)
    weights_load = load_profile / load_profile.sum()

    ren_carriers = [
        "onwind", "offwind-ac", "offwind-dc", "offwind-float", "solar", "ror", "hydro"
    ]
    gen = n.generators.copy()
    gen = gen[gen.carrier.isin(ren_carriers) & gen.bus.str.startswith("FR0")]
    ren_capacity = gen.groupby("bus").p_nom.sum()
    ren_capacity = ren_capacity.reindex(fr_load_buses, fill_value=0.0)
    if ren_capacity.sum() > 0:
        weights_ren = ren_capacity / ren_capacity.sum()
    else:
        weights_ren = pd.Series(1.0 / len(fr_load_buses), index=fr_load_buses)

    for _, row in df.iterrows():
        tech = row["tech"]
        typ = row["type"]
        carrier = row["carrier"]
        p_nom_total = row["p_nom"]
        e_nom_total = row.get("e_nom", np.nan)
        eff = row.get("efficiency", 1.0)
        eff_store = row.get("efficiency_store", 1.0)
        eff_dispatch = row.get("efficiency_dispatch", 1.0)
        cost = row.get("marginal_cost", 0.0)

        if typ == "link":
            if tech.lower() == "p2g":
                min_p_nom = MIN_P_NOM.get("p2g", 0.0)
                allocations = allocate_with_minimum(weights_ren, p_nom_total, min_p_nom)
                for bus_name, p_nom in allocations.items():
                    h2_bus = f"h2_store_{bus_name}"
                    link_name = f"{tech}_{bus_name}"
                    if h2_bus not in n.buses.index:
                        n.add("Bus", h2_bus, carrier="H2")
                        n.buses.loc[h2_bus, ['x', 'y']] = n.buses.loc[bus_name, ['x', 'y']]
            ...
            elif tech.lower().startswith("g2p"):
                min_p_nom = MIN_P_NOM.get(tech, 0.0)
                allocations = allocate_with_minimum(weights_load, p_nom_total, min_p_nom)
                for bus_name, p_nom in allocations.items():
                    h2_bus = f"h2_store_{bus_name}"
                    link_name = f"{tech}_{bus_name}"
                    if h2_bus not in n.buses.index:
            ...
            for bus_name, w in weights_ren.items():
                tech_bus = f"{tech}_{bus_name}"
                if tech_bus not in n.buses.index:
                    n.add("Bus", tech_bus, carrier="H2")
                    n.buses.loc[tech_bus, ["x", "y"]] = n.buses.loc[bus_name, ["x", "y"]]
                store_name = f"{tech}_{bus_name}"
                if store_name not in n.stores.index:
                    n.add("Store",
                          name=store_name,
                          bus=tech_bus,
                          e_nom=(w * e_nom_total) if not pd.isna(e_nom_total) else 0.0,
                          e_cyclic=True,
                          carrier=carrier,
                          marginal_cost=cost,
                          tech=tech)

        elif typ == "storage_unit":
            for bus_name, w in weights_load.items():
                batt_name = f"{tech}_{bus_name}"
            ...
            raise ValueError(f"Unknown flexibility type: {typ}")
...
if __name__ == "__main__":
    ...
    add_flexibility(
        n=n,
        flexibility_csv=snakemake.input.flexibility_technologies,
